...
public class RetornarEstadoPedidoAction implements Action{
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
        String codigoPedido = request.getParameter("id");
        
        try{
            if(codigoPedido == null || codigoPedido.equals("")){
                request.setAttribute("alteracaoEstado", "Não possui recuperação.");
            ...
            pedido.restoreFromMemento(EstadosSalvosSingleton.getInstance().getListaEstados().get(0));
